 ...
 * 系统事件广播监听(新版)
 */
public enum SystemEventHelper2 {
    INSTANCE;
    ...
    public void init() {
        ...
        AppManager.INSTANCE.getApplication().registerReceiver(mSysEventReceiver, intentFilter);
        //移动网络信号监听
        TelephonyManager mTelephonyManager = (TelephonyManager) AppManager.INSTANCE.getApplication().getSystemService(Context.TELEPHONY_SERVICE);
        mTelephonyManager.listen(mPhoneStateListener, PhoneStateListener.LISTEN_SIGNAL_STRENGTHS);
        //网络连接状况监听
        ConnectivityManager connectivityManager = AppManager.INSTANCE.getApplication().getSystemService(ConnectivityManager.class);
    ...
    private void initSystemEventListener(OnSystemEventListener listener) {
        if (listener != null) {
            ...
            //获取电池电量/充电状态
            BatteryManager batteryManager = (BatteryManager) AppManager.INSTANCE.getApplication().getSystemService(Context.BATTERY_SERVICE);
    ...
    public void clear() {
        ...
        AppManager.INSTANCE.getApplication().unregisterReceiver(mSysEventReceiver);
    }
    ...
    public static int getWifiNetworkRSSILevel(int level) {
        WifiManager wifiManager = (WifiManager) AppManager.INSTANCE.getApplication()
    ...
    public static boolean hasSimCard() {
        TelephonyManager telMgr = (TelephonyManager) AppManager.INSTANCE.getApplication().getSystemService(Context.TELEPHONY_SERVICE);
